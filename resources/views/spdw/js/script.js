...
/* ---- Analog dial gauge helpers ----
   Each dial is a semicircle arc (path) + a needle (line) rotating
   about the arc's center (60,65), like a physical meter face. */
function setDial(prefix, pct) {
  ...
  arc.style.strokeDashoffset = len * (1 - clamped/100);
  const angle = -90 + (clamped/100)*180; // -90deg (left) .. +90deg (right)
  needle.style.transform = `rotate(${angle}deg)`;
}
function initDials() {
  setDial('tds', (142/400)*100);
  setDial('ph',  (7.2/14)*100);
}
...
function simulateSensor() {
  ...
  setDial('tds', (tds/400)*100);
  setDial('ph',  (ph/14)*100);

  const fill = document.getElementById('vol-fill');
  fill.style.height = vol+'%';
  if (vol > 50)       fill.style.background = 'linear-gradient(180deg,var(--water),var(--water-dk))';
  else if (vol > 25)  fill.style.background = 'linear-gradient(180deg,var(--brass-lt),var(--warn))';
  else                fill.style.background = 'linear-gradient(180deg,#D98A7D,var(--danger))';

  const tdsOk = tds < 300;
  const phOk  = ph >= 6.5 && ph <= 8.5;
  const volOk = vol > 10;

  setStatus('tds-val','tds-status', tdsOk);
  setStatus('ph-val','ph-status',   phOk);
...
function renderPengaduanHistory() {
  ...
  const prioColor = { Tinggi:'<i class="fa-solid fa-circle" style="color:var(--danger)"></i>', Sedang:'<i class="fa-solid fa-circle" style="color:var(--warn)"></i>', Rendah:'<i class="fa-solid fa-circle" style="color:var(--ok)"></i>' };
...
function bC(v){return v>50?'#4C7A5C':v>25?'#B4802E':'#A6392D';}
function bT(v){return v>50?'#4C7A5C':v>25?'#B4802E':'#A6392D';}
...
function renderPengaduanAdmin() {
  ...
  if (filtered.length === 0) {
    list.innerHTML = `<div style="text-align:center;padding:40px;color:var(--muted);">
      <div style="font-size:44px;margin-bottom:12px;color:var(--brass-dk);"><i class="fa-solid fa-inbox"></i></div>
      <div style="font-weight:700;font-size:15px;color:var(--ink);font-family:var(--f-display);">Belum ada pengaduan</div>
      <div style="font-size:12.5px;margin-top:4px;">Laporan dari pengguna akan muncul di sini</div>
    </div>`;
    return;
  }
    ...
    <div class="peng-admin-card">
      ...
      <div class="peng-prio">
        ...
        <div style="margin-top:8px;display:flex;flex-direction:column;gap:4px;">
          ${p.status==='Baru'?`<button onclick="setPengStatus(${pengaduanList.indexOf(p)},'Diproses')" style="font-size:11px;padding:5px 10px;border-radius:6px;border:1px solid var(--warn);background:var(--warn-bg);color:var(--warn);cursor:pointer;font-weight:700;font-family:var(--f-mono);">Proses</button>`:''}
          ${p.status!=='Selesai'?`<button onclick="setPengStatus(${pengaduanList.indexOf(p)},'Selesai')" style="font-size:11px;padding:5px 10px;border-radius:6px;border:1px solid var(--ok);background:var(--ok-bg);color:var(--ok);cursor:pointer;font-weight:700;font-family:var(--f-mono);">Selesai</button>`:''}
        </div>
      </div>
    </div>
  `).join('');
